...
def _get_generated_text(case: Dict[str, Any]) -> str:
    ...
    if isinstance(gt, str):
        return gt.strip()
    # Some endpoints store the output under a different key; allow it as a fallback.
...
class RuleEngine:
    ...
    def __init__(
        ...
        *,
        ...
        important_spec: Optional[ImportantFieldsSpec] = None,
    ):
        # Test-case schema requirements (project-specific override supported)
        self.required_fields_override = required_fields_override
        ...
        self.important_spec = important_spec or ImportantFieldsSpec.load_from_env()
    ...
    def check_case(self, case: Dict[str, Any]) -> List[RuleResult]:
        ...
        required_fields = self.required_fields_override or self._infer_required_fields(case)
        ...
        # ---- Project-specific important terms must be mentioned (optional but can be mandatory)
        results.extend(self._check_must_mention_terms(gen_text, case))
    ...
    def _infer_required_fields(self, case: Dict[str, Any]) -> List[str]:
        """Infer schema required fields from the testcase shape.

        This keeps backward compatibility for the original talent/company schema,
        while supporting other endpoints (e.g., lab + company).

        Note: We intentionally do NOT support the misspelled key "company".
        Please use "company".
        """
        # Prefer explicit endpoint shapes first.

        # 1) Lab-style endpoint
        if "lab" in case:
            if "company" in case:
                return ["lab", "company", "generated_text"]
            return ["lab", "generated_text"]

        # 2) Original hiring-style endpoint (only require talent if it exists)
        if "talent" in case:
            if "company" in case:
                return ["talent", "company", "generated_text"]
            return ["talent", "generated_text"]

        # 3) Company-only endpoints
        if "company" in case:
            return ["company", "generated_text"]

        # 4) Fallback
        return ["generated_text"]

    def _check_must_mention_terms(self, gen_text: str, case: Dict[str, Any]) -> List[RuleResult]:
        """Enforce that important context terms are mentioned in output (project-specific).

        This is useful when you want strict alignment between injected structured data and output.
        Config is loaded from IMPORTANT_FIELDS_SPEC.
        """
        terms = self.important_spec.derive_must_mention_terms(case)
        if not terms:
            return [RuleResult(name="accuracy_must_mention_terms", passed=True, reason="no spec", mandatory=False)]

        # Normalize output for matching (same normalization policy as spec)
        import unicodedata
        out = gen_text
        if self.important_spec.normalize_nfkc:
            out = unicodedata.normalize("NFKC", out)
        if self.important_spec.normalize_lower:
            out = out.lower()
        ...
        ok = True
        reason = None
        if self.important_spec.must_mention_mode == "all":
            ok = len(missing) == 0
            if not ok:
                reason = f"missing_terms={missing}"
        else:
            # any
            ok = len(hits) >= max(1, self.important_spec.must_mention_min_count)
            if not ok:
                reason = f"hits={len(hits)} < min_count={self.important_spec.must_mention_min_count}"

        return [
            RuleResult(
                name="accuracy_must_mention_terms",
                passed=ok,
                reason=reason,
                mandatory=bool(self.important_spec.must_mention_mandatory),
            )
        ]
