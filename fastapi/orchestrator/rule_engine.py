...
class RuleEngine:
    ...
    def check_case(self, case: Dict[str, Any]) -> List[RuleResult]:
        ...
        for f in required_fields:
            ok = f in case
            # tolerate "company" vs "comapny" (some endpoints use the typo)
            if not ok and f == "company" and "comapny" in case:
                ok = True
            if not ok and f == "comapny" and "company" in case:
                ok = True
    ...
    def _infer_required_fields(self, case: Dict[str, Any]) -> List[str]:
        ...
        while supporting other endpoints (e.g., lab + comapny/company).
        """
        if "talent" in case or "company" in case:
            return ["talent", "company", "generated_text"]
        if "lab" in case and ("comapny" in case or "company" in case):
            return ["lab", "comapny" if "comapny" in case else "company", "generated_text"]
